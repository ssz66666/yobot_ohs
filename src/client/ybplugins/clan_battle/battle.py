...
class ClanBattle:
    ...
    def challenge(self,
        ...
        current_boss_num = group.boss_num
        if defeat:
            if group.boss_num == 5:
                group.boss_num = 1
                group.boss_cycle += 1
        ...
        Clan_subscribe.delete().where(
            Clan_subscribe.gid == group_id,
            Clan_subscribe.qqid == qqid,
            Clan_subscribe.subscribe_item == current_boss_num,
        ).execute()
        ...
        if match_num == 1:  # 创建
            ...
            for m in subscribers:
                ...
                if m.get('message'):
                    reply += '：' + m['message']
            return reply
        elif match_num == 26: # 查尾刀
            group = Clan_group.get_or_none(group_id=group_id)
            if group is None:
                return 
            d, t = pcr_datetime(area=group.game_server)
            report = self.get_report(group_id, None, None, d)
            members = {}
            progress = {}
            halves = []
            for r in report:
                if r['qqid'] not in members:
                    members[r['qqid']] = []
                members[r['qqid']].append(r)
            for m, rs in members.items():
                p = 0
                for r in rs:
                    if r['is_continue']:
                        p += 1
                    else:
                        if r['health_ramain'] > 0:
                            p += 2
                        else:
                            p += 1
                progress[m] = p
            for m, p in progress.items():
                if p % 2 != 0:
                    halves.append(members[m][-1])
            if len(halves) == 0:
                return '没有未完成尾刀'
            else:
                reply = '未完成尾刀：\n'
                for r in halves:
                    reply += '\n' + self._get_nickname_by_qqid(r['qqid'])
                    reply += ' ' + str(r['cycle']) + '-' + str(r['boss_num'])
                    reply += ' ' + str(r['damage'])
